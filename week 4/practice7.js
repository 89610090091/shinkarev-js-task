...
function remove(array, index) {
    ...
    if (index === array.length - 1) {
        // на случай если нужно удалить последний элемент
        result = array.slice();
        result.pop();
    } else {
        result = result.concat(array.slice(0, index));
...
function indexOfFruit(fruitList, fruitName) {
    // приводим каждый элемент к нижнему регистру
    // ['яблоко', 'апельсин', 'груша', 'ананас']
    return fruitList.map((fruit) => fruit.toLowerCase())
        .indexOf(fruitName.toLowerCase()); // ищем индекс 'ананас'а
}
